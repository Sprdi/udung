...
export function SagPage() {
  ...
  const [isAdmin, setIsAdmin] = useState(false); // State untuk menyimpan status apakah pengguna adalah admin

  // UseEffect untuk mengambil data saat komponen dimount dan di balikan urutan
  useEffect(() => {
    getSags((data) => {
      if (data === null) {
        // Tampilkan layar putih atau arahkan ke halaman login
  ...
  // Contoh pengaturan status admin
  useEffect(() => {
    ...
    if (userRole) {
      ...
      console.log("User Role not set or user is not logged in.");
      // Handle user not logged in or role not set, e.g., redirect to login
    }
  }, []);
